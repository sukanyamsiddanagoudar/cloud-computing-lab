\# Hypervisor-2 — VMware Workstation



\## 1. System Configuration



| Parameter        | Value              |

| ---------------- | ------------------ |

| Hypervisor       | VMware Workstation |

| Hypervisor Type  | Type-2             |

| Operating System | Ubuntu 24.04.5 LTS |

| vCPU             | 2                  |

| RAM              | 2 GB               |

| Disk             | 20 GB              |

| Network          | NAT                |



\## 2. CPU Information



| Parameter        | Value                                            |

| ---------------- | ------------------------------------------------ |

| CPU Model        | 12th Gen Intel(R) Core(TM) i5-1235U              |

| CPU Architecture | x86\_64                                           |

| CPU Cores        | 2 (1 socket × 2 cores/socket)                    |

| CPU Threads      | 2 (1 thread/core)                                |

| CPU Frequency    | Not shown in captured output (BogoMIPS: 4992.00) |



\## 3. Memory Information



| Parameter        | Value   |

| ---------------- | ------- |

| Total Memory     | 1.9 GiB |

| Used Memory      | 1.1 GiB |

| Free Memory      | 128 MiB |

| Available Memory | 796 MiB |



\## 4. Disk Information



| Parameter        | Value     |

| ---------------- | --------- |

| Filesystem       | /dev/sda2 |

| Total Disk Space | 20 GB     |

| Used Space       | 9.8 GB    |

| Available Space  | 8.8 GB    |

| Usage            | 53%       |



\## 5. CPU Benchmark — Sysbench



| Parameter         | Value        |

| ----------------- | ------------ |

| Benchmark         | Sysbench CPU |

| CPU Max Prime     | 20000        |

| Total Events      | 12,911       |

| Total Time        | 10.0005 s    |

| Events Per Second | 1290.89      |

| Minimum Latency   | 0.68 ms      |

| Average Latency   | 0.77 ms      |

| Maximum Latency   | 8.52 ms      |



\## 6. Resource Utilization



| Resource      | Value                                                       |

| ------------- | ----------------------------------------------------------- |

| CPU Usage     | 1.7% user, 1.0% system, 97.0% idle                          |

| Memory Usage  | 1125.5 MiB used / 1915.2 MiB total (\~58.8%)                 |

| Disk Usage    | 53% (root partition, from `df -h`)                          |

| Network Usage | Not captured in screenshots (NAT mode, no traffic measured) |



\## 7. Result Summary



| Metric             | Observed Value                             |

| ------------------ | ------------------------------------------ |

| CPU Benchmark      | Sysbench prime calculation (limit 20000)   |

| Execution Time     | 10.0005 s                                  |

| Events Per Second  | 1290.89                                    |

| Average Latency    | 0.77 ms                                    |

| CPU Utilization    | \~3% active (97% idle) during test snapshot |

| Memory Utilization | \~58.8% (1.1 GiB / 1.9 GiB)                 |



\## 8. Screenshots



The experimental output screenshots are available in the `screenshots` folder.



